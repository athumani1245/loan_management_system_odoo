...
#             loan_count = loan_obj.sudo().search([('company_id','=',self.env.user.company_id.id)])
            self._cr.execute("select count(id) from account_loan where company_id={} and state='submit'".format(self.env.user.company_id.id))
...
#             loan_cancel_count = loan_obj.sudo().search([('state', '=', 'cancel')])
    ...
    def _get_total_amount(self):
        ...
        print("At the end=====================================",total_out)
        currency_name = self.env.user.company_id.currency_id.name
        self.total_amt = str(format(round(total,2),",")) + ' (' + currency_name + ')'
        self.total_out_amt = str(format(round(total_out,2),",")) + ' (' + currency_name + ')'
...
#                 print("dis.bill_date-------------------> ",dt.year,type(dt.year),type(dis.bill_date),dis.bill_date)
    ...
    def get_out_datas(self):
        ...
        fees = 0.00
        if not (self._uid == SUPERUSER_ID or self.env.user.has_group('base.group_system')):
            loan_obj = self.env['account.loan'].sudo().search([('company_id','=',self.env.user.company_id.id),('state','in',['partial','approved'])])
        else:
        ...
        for loan in loan_obj:
            ...
            #print("At the end the total principal out=====================================",pri)
...
#                 dis_tot += dis.disbursement_amt
    ...
    def get_amount_dict(self):
        ...
        for loan in loan_obj:
            for ins in loan.installment_id:
                if ins.date:
                    if datetime.strptime(str(ins.date), "%Y-%m-%d") <= date_new and ins.state != 'paid':
                        if ins.state == 'open' and ins.outstanding_prin == 0:
                            pri += 0
                        else:
