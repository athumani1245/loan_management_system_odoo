...
#     prooftypes = fields.Many2many('account.loan.proof.type', 'loantype_prooftype_rel', 'order_line_id', 'tax_id', 'Proof')
    ...
    account_id = fields.Many2one('account.account', string="Excess Payment Acc")
    process_fee = fields.Float('Processing Fee in %', digits=(12, 2), track_visibility='onchange')
    proc_fee = fields.Many2one('account.account', method=True, string="Processing fee Acc", company_dependent=True,  track_visibility='onchange')

    tra_fee = fields.Float('Additional of title fee', digits=(12, 2), track_visibility='onchange')
    tra_fee_acc = fields.Many2one('account.account', method=True, string="Additional of title Acc", company_dependent=True,  track_visibility='onchange')

    insure_fee = fields.Float('Insurance Fee in %', digits=(12, 2), track_visibility='onchange')
    insure_fee_acc = fields.Many2one('account.account', method=True, string="Insurance fee Acc", company_dependent=True,  track_visibility='onchange')


    tracking_fee = fields.Float('Car tracking fee', digits=(12, 2), track_visibility='onchange')
    tracking_fee_acc = fields.Many2one('account.account', method=True, string="Car tracking Fee Acc", company_dependent=True,  track_visibility='onchange')
    parking_fee = fields.Float('Car Parking fee', digits=(12, 2), track_visibility='onchange')
    parking_fee_acc = fields.Many2one('account.account', method=True, string="Car Parking Fee Acc", company_dependent=True,  track_visibility='onchange')
    ...
    def _compute_rule(self):
        ...
        if self.amount_select == 'fix':
            try:
                ...
                raise UserError(_('Wrong quantity defined for  rule %s (%s).') % (self.name, self.code))
