...
class LoanCustomer(models.Model):
    ...
    nida_id = fields.Char('Nida Id')
    other_id = fields.Char('Other Id')
    provider_id=fields.Selection(selection=[('NIDA', 'NIDA'),('TRA', 'TRA'), ('IMGIGRATION', 'IMIGRATION')
                                                          ], track_visibility='onchange', string='ID Provider')

    reg_no = fields.Char('Registration No')
    tin_no = fields.Char('TIN No')
    birthdate = fields.Date('Birth Date')

    spousename=fields.Char('Name of Spouse')
    spousephone1=fields.Char('Spouse Phone1')
    spousephone2=fields.Char('Spouse Phone2')
    spousenational_id=fields.Char('Spouse National ID')
    spousenational_any_id=fields.Char('Spouse Other ID')
    #spousenational_id_provider=fields.Char('Spouse ID Provider')
    spousenational_id_provider=fields.Selection(selection=[('NIDA', 'NIDA'),('TRA', 'TRA'), ('IMGIGRATION', 'IMIGRATION')
                                                          ], track_visibility='onchange',string='ID Provider' )

    spousejob=fields.Char('Spouse Job')
    region=fields.Char('Region')
    district=fields.Char('District')
    ward=fields.Char('Ward')
    street=fields.Char('Street')
    plot=fields.Char('Plot No')
    block=fields.Char('Block No')
    house=fields.Char('House No')
    lardloadname = fields.Char('Larnd Loard Name')
    lardloadno1 = fields.Char('Larnd Loard mobile1')
    lardloadno2 = fields.Char('Larnd Loard mobile2')
    mjumbe=fields.Char('Mjumbe wa mtaa')
    mwenyekiti=fields.Char('Local Gov Chairperson')
    balozi=fields.Char('Balozi wa mtaa')

    bankname=fields.Char('Bank Name')
    bankacc=fields.Char('Account No')
    bankbranch=fields.Char('Bank Branch')
    bankcode=fields.Char('Branch Code')

class LoanCustomerNidaiD(models.Model):
    _name = 'res.users'
    _inherit = 'res.users'
    nida_id = fields.Char('Nida Id')

class Cust_Loan_Gurantor(models.Model):

    _name = 'cust.gurantor'
    
    name = fields.Char('Gurantor Name')
    image = fields.Binary()
    G_district = fields.Char('Gurantor District')
    G_region = fields.Char('Gurantor Region')
    G_ward = fields.Char('Gurantor Ward')
    G_street = fields.Char('Gurantor Street')
    G_blockNo = fields.Char('Gurantor Block NO')
    G_plotNo = fields.Char('Gurantor Plot No')
    G_houseNo = fields.Char('Gurantor House')
    G_phone1 = fields.Char('Gurantor Mobile1')
    G_phone2 = fields.Char('Gurantor Mobile2')
    local_gov = fields.Char('Local Goverment Chairperson')
...
class CollateralLine(models.Model):
    ...
    loan_id = fields.Many2one('account.loan')
    type_id = fields.Many2one('collateral.type','Type')
    value = fields.Float('Value')
    coll_name = fields.Char('Collateral Name')
    coll_owner = fields.Char('Owner')
    coll_model = fields.Char('Model')
    coll_make = fields.Char('Make')
    coll_chasis = fields.Char('Chasis No')
    coll_reg = fields.Char('Registration No')

    coll_year = fields.Char('Manf Year')
    coll_color = fields.Char('Car Color')
    last_date = fields.Date('Last Valuation')
    expiry_date = fields.Date('Expiry Date')
    is_registered = fields.Boolean('Is Registered')
...
#     auto_id = fields.Integer('Auto Id', size=32, default=lambda self: self.env['ir.sequence'].next_by_code('loan.id'), track_visibility='onchange')
    name = fields.Text('Purpose', size=128, required=True, track_visibility='onchange')
    reject_resons = fields.Text('Rejection Resons', size=128, )
    partner_id = fields.Many2one('res.partner', 'Customer', required=True,track_visibility='onchange')
    proof_1 = fields.Many2one('cust.gurantor', 'Guarantor/Co-Signer 1', track_visibility='onchange')
    proof_2 = fields.Many2one('cust.gurantor', 'Guarantor/Co-Signer 2', track_visibility='onchange')
...
#         'loan_period':fields.selection(_loan_period_get,'Loan Period',select=True, required=True),
    process_fee_percent = fields.Float(related="loan_type.process_fee")
    process_fee = fields.Float(compute='compute_process_fee_ammount',store=True)


    insure_fee_percent = fields.Float(related="loan_type.insure_fee")
    insure_fee = fields.Float('Insurence fee',compute='compute_insure_fee_ammount',store=True)
    
    tra_compute = fields.Float(related="loan_type.tra_fee")
    tra_fee = fields.Float('Additional of title fee',compute='compute_tra_fee_ammount',store=True)
    

    tracking_compute = fields.Float(related="loan_type.tracking_fee")
    tracking_fee = fields.Float('Car tracking fee',compute='compute_tracking_fee_ammount',store=True)

   # tracking_fee = fields.Float(related="loan_type.tracking_fee")
    parking_compute = fields.Float(related="loan_type.parking_fee")
    parking_fee = fields.Float('Car Parking fee',compute='compute_parking_fee_ammount',store=True)

    other_deduct = fields.Float('Other Deductions')
    other_deduct_acc = fields.Many2one('account.account', method=True, string="Other Deduction A/c", company_dependent=True,  track_visibility='onchange')

    loan_period = fields.Many2one('loan.installment.period', 'Loan Period', required=True, track_visibility='onchange')
    loan_amount = fields.Float('Approved Amount', digits=(12, 2), track_visibility='onchange')
    approve_amount = fields.Float('Disbursement Amount', digits=(12, 2), readonly=True, track_visibility='onchange')
    proposed_amount = fields.Float('Proposed Amount', digits=(12, 2), track_visibility='onchange')

    #cr_manager = fields.Many2one('res.users', string='Credit Manager')
...
#   'interest_rate': fields.function(compute_loan_interest,string='Interest Rate', track_visibility='always',multi='all',store=True),
    ...
    department =fields.Selection([('mbs','Micro Business Solutions'),('sme','SME Growth')],string="Department", default="mbs")
    repayment_basis = fields.Selection([('disbursed_amt','Disbursed Amount'), ('sanctioned_amt','Sanctioned Amount')], "Repayment Basis", default="disbursed_amt")

    is_refugee = fields.Selection([('ref','Refugee'),('non_ref','Non Refugee')])
    
    
    apply_date = fields.Date('Application Date', states={'draft':[('readonly', False)]}, default=time.strftime('%Y-%m-%d'),track_visibility='onchange')
    ...
    return_type = fields.Selection([
       ...
       ('cheque', 'By Cheque'),
       ('automatic', 'By Mobile'),
    ], 'Payment Type', index=True, default='cash', track_visibility='onchange')
...
#     running_loan = fields.Many2many('account.loan', 'account_loan_running', 'loan_id', 'auto_id', 'Current Loans',track_visibility='onchange')
    ...
    interest = fields.Float('Interest Amount', digits=(12, 2),track_visibility='onchange', copy=False)
    voucher_id = fields.Many2one('account.voucher', 'Voucher', readonly=True, track_visibility='onchange')
    notes = fields.Text('Dear Officer , Please writte Car parking details if not available , indicate N/A', track_visibility='onchange')
    ...
    proc_fee = fields.Many2one(related="loan_type.proc_fee")
    tra_fee_acc = fields.Many2one(related="loan_type.tra_fee_acc")
    tracking_fee_acc = fields.Many2one(related="loan_type.tracking_fee_acc")
    parking_fee_acc = fields.Many2one(related="loan_type.parking_fee_acc")

    insure_fee_acc = fields.Many2one(related="loan_type.insure_fee_acc")
    ...
    loan_amt = fields.Float('Amount to Approve ', digits=(12, 2), required=True, track_visibility='onchange')
    req_amt = fields.Float('Amount Requested', digits=(12, 2))
    ...
    is_collateral = fields.Boolean(default=True)
...
#     total_late_fees_paid = fields.Float(compute='_compute_total_late_fees_paid')
    ...
    def fetch_last_installment_date(self):
        vals = {}
        installment = self.env['account.loan.installment'].search([('loan_id', '=', self.id)])
        # if installment:
        for rec in installment:
            vals.update({
                'date': rec.date,
                'total_out_prin': sum(installment.mapped('outstanding_prin')),
                'out_prin': rec.outstanding_prin,
                'out_int': rec.outstanding_int,
                'total_out_int': sum(installment.mapped('outstanding_int')),
                'out_fees': rec.outstanding_fees,
                'total_out_fees': sum(installment.mapped('outstanding_fees')),
                'late_fee': rec.late_fee,
                'due_principal': rec.due_principal,
                'total_due_principal': sum(installment.mapped('due_principal')),
                'total_due_interest': sum(installment.mapped('due_interest')),
                'total_due_penalty': sum(installment.mapped('late_fee')),
                'due_interest': rec.due_interest,
                'capital': rec.capital,
                'total_capital': sum(installment.mapped('capital')),
                'interest': sum(installment.mapped('interest')),
                'total_fees': sum(installment.mapped('fees')),
                'fees': rec.fees,
            })
        return vals

    def fetch_repayment_details(self):
        vals = {}
        installment = self.env['account.loan.repayment'].search([('loan_id', '=', self.id)])
        # if installment:
        for rec in installment:
            vals.update({
                'release_number': rec.release_number.name,
                'pay_date': rec.pay_date,
            })
        return vals

    def fetch_disbursement_details(self):
        vals = {}
        installment = self.env['account.loan.disbursement'].search([('loan_id', '=', self.id)])
        # if installment:
        for rec in installment:
            vals.update({
    ...
    @api.depends('process_fee_percent','loan_amount')
    def compute_process_fee_ammount(self):

        self.process_fee=(self.process_fee_percent / 100) * self.loan_amount
        return self.process_fee

    @api.depends('insure_fee_percent','loan_amount')
   # @api.onchange('loan_amount')
    def compute_insure_fee_ammount(self):
        self.insure_fee=(self.insure_fee_percent / 100) * self.loan_amount
        return self.insure_fee

    @api.depends('tra_compute')
   # @api.onchange('loan_amount')
    def compute_tra_fee_ammount(self):
        self.tra_fee=self.tra_compute
        return self.tra_fee

    @api.depends('tracking_compute')
    def compute_tracking_fee_ammount(self):
        self.tracking_fee=self.tracking_compute
        return self.tracking_fee

    @api.depends('parking_compute')
    def compute_parking_fee_ammount(self):
        self.parking_fee=self.parking_compute
    ...
    def onchange_loan_amount(self):
        self.compute_insure_fee_ammount()
        self.compute_process_fee_ammount()
...
#                     for int_version_line in int_version.interestversionline_ids:
    ...
    def loan_interest_get(self):
        for loan in self:
            self.write({'approve_amount':loan.loan_amount - loan.process_fee - loan.insure_fee - loan.tra_fee - loan.tracking_fee - loan.parking_fee-loan.other_deduct, 'approve_date':time.strftime('%Y/%m/%d')})
        return True
...
#         print("\n\n\n Date UPdate ::: ",date_update)
        ...
        interest = round(((disbursed_amt / 100 * self.interest_rate)*(float(total_installment)))/float(total_installment),2)
...
#         print("\n\n\n Date UPdate ::: ",date_update)
    ...
    #                         int_rate = self.calculate_eir(inter_rate, self.loan_amount, self.total_installment/ 12 , inter_rate, 0) 
            ...
            check_list = []
            try:
                installment = round(((total * (rate_interest)) / (1 - ((1 + rate_interest) ** -(total_installment)))))
...
#                 date_update = date_update.replace(year = new_year)
            ...
            for i in range(numbering,self.total_installment+1):
                ...
                if self.loan_type.calculation == 'reducing':
                    interest_month = round(((total * rate_interest)), 2)
...
#                         int_rate = self.calculate_eir(inter_rate, self.loan_amount, self.total_installment / 12 , inter_rate, 0)
        ...
        if gp_fee == gp_int == gp_principal:  
            ...
            try:
                installment = round(((total * (rate_interest )) / (1 - ((1 + rate_interest ) ** -(self.total_installment)))))
            except ZeroDivisionError:
                installment = 0
        ...
        for i in range(numbering, self.total_installment+1):
            grace_int = round(((main_tot * rate_interest)), 2)
            principle_amount = 0.0
            interest_month = 0.0
            is_capital_pay = False
            int_for_prin = round(((total * rate_interest)), 2)
            ...
            interest = round(((100000 / 100 * self.interest_rate)*(float(total_installment)))/float(total_installment),2)
            ...
            if gp_fee == gp_int == gp_principal:
                if self.loan_type.calculation == 'flat':
                    if self.repayment_basis == 'sanctioned_amt':
                        ...
                        interest = round(((updated_loan_amount / 100 * self.interest_rate)*(float(self.total_installment)))/float(self.total_installment),2)
                    else:
                        capital = round(disbursed_amt / float(self.total_installment - global_dict.get('principal')),2)
                        interest = round(((disbursed_amt / 100 * self.interest_rate)*(float(self.total_installment)))/float(self.total_installment),2)
...
#                             print ("=============v===========",updated_loan_amount)
                            interest = round(((updated_loan_amount / 100 * self.interest_rate)*(float(total_installment)))/float(total_installment),2)
                ...
                if key_min == 'principal':
                    ...
                    if self.loan_type.calculation == 'flat':
                        ...
                        if flag:
                            ...
                            int_month = round(((pr_total * rate_interest) ), 2)
                        principle_amount = round(installment - int_for_prin, 2)
                    else:
                        if self.repayment_basis == 'sanctioned_amt':                
                            capital = round(updated_loan_amount / float(self.total_installment - global_dict.get('principal')),2)
...
#                 self.env['payment.schedule.line'].create(vals)
    ...
    def create_moves(self, disburse_amt, disburse_date, flag, currency_id):
        ...
        name_title_Additional_fees = "Additional of title Fee Collected For: "
        name_car_tracking_fees = "Car Tracking Fee Collected For: "
        name_car_parking_fees = "Car Parking Fee Collected For: "
        name_other_deduction_fees = "Other Deduction from loan: "

        name_insurance_fees = "Insurance Fee Collected For: "
        name_bank_deposite = "Amount Deposited In Customer Account For: "
        date_new = disburse_date
        move_lines_cr = {}
        move_lines_dr = {}
        processing_fee = {}
        title_Additional_fee = {}
        car_tracking_fee = {}
        car_parking_fee = {}
        other_deduction_fee = {}
        insurance_fee = {}
        ...
        if self.loan_id:
            ...
            ##calculation for Insurance fee .............................
            if self.insure_fee and flag:
                if not self.insure_fee_acc:
                    raise UserError(_('Please Configure Insurance Fee Journal'))
                
                if company_curren:
                    if currency_id.id != company_curren.id:
                        amount_currency = self.insure_fee
                        amount = company_curren.with_context(date=date_new).compute(self.insure_fee, currency_id)
                        fee_cnt = amount
                    else:
                        amount_currency = False
                if not amount_currency:
                    insurance_fee.update({
                                            'account_id':self.insure_fee_acc.id,
                                            'name':name_insurance_fees+self.loan_id,
                                            'debit':0.0,
                                            'credit':self.insure_fee,
                                            'partner_id':self.partner_id.id,
                        })
                else:
                    insurance_fee.update({
                ...
                list_mv_line.append((0, 0, insurance_fee))



              ##calculation for Additioan title TRA fee .............................
            if self.tra_fee and flag:
                if not self.tra_fee_acc:
                    raise UserError(_('Please Configure  fee for additional of title Journal'))
                
                if company_curren:
                    if currency_id.id != company_curren.id:
                        amount_currency = self.tra_fee
                        amount = company_curren.with_context(date=date_new).compute(self.tra_fee, currency_id)
                        fee_cnt = amount
                    else:
                        amount_currency = False
                if not amount_currency:
                    title_Additional_fee.update({
                                            'account_id':self.tra_fee_acc.id,
                                            'name':name_title_Additional_fees+self.loan_id,
                                            'debit':0.0,
                                            'credit':self.tra_fee,
                                            'partner_id':self.partner_id.id,
                        })
                ...
                list_mv_line.append((0, 0, title_Additional_fee))


             ##calculation for Car tracking fee .............................
            if self.tracking_fee and flag:
                if not self.tracking_fee_acc:
                    raise UserError(_('Please Configure  fee for Tracking Car Journal'))
                
                if company_curren:
                    if currency_id.id != company_curren.id:
                        amount_currency = self.tracking_fee
                        amount = company_curren.with_context(date=date_new).compute(self.tracking_fee, currency_id)
                        fee_cnt = amount
                    else:
                        amount_currency = False
                if not amount_currency:
                    car_tracking_fee.update({
                                            'account_id':self.tracking_fee_acc.id,
                                            'name':name_car_tracking_fees+self.loan_id,
                                            'debit':0.0,
                                            'credit':self.tracking_fee,
                                            'partner_id':self.partner_id.id,
                        })
                else:
                    car_tracking_fee.update({
                ...
                list_mv_line.append((0, 0, car_tracking_fee))



             ##calculation for Car Parking fee .............................
            if self.parking_fee and flag:
                if not self.parking_fee_acc:
                    raise UserError(_('Please Configure  fee for Car Parking Journal'))
                
                if company_curren:
                    if currency_id.id != company_curren.id:
                        amount_currency = self.parking_fee
                        amount = company_curren.with_context(date=date_new).compute(self.parking_fee, currency_id)
                        fee_cnt = amount
                    else:
                        amount_currency = False
                if not amount_currency:
                    car_parking_fee.update({
                                            'account_id':self.parking_fee_acc.id,
                                            'name':name_car_parking_fees+self.loan_id,
                                            'debit':0.0,
                                            'credit':self.parking_fee,
                                            'partner_id':self.partner_id.id,
                        })
                else:
                    car_parking_fee.update({
                ...
                list_mv_line.append((0, 0, car_parking_fee))

            ##calculation for Other Deduction  fee .............................
            if self.other_deduct and flag:
                if not self.other_deduct_acc:
                    raise UserError(_('Please Configure  fee for Other deduction Ledger Journal'))
                
                if company_curren:
                    if currency_id.id != company_curren.id:
                        amount_currency = self.other_deduct
                        amount = company_curren.with_context(date=date_new).compute(self.other_deduct, currency_id)
                        fee_cnt = amount
                    else:
                        amount_currency = False
                if not amount_currency:
                    other_deduction_fee.update({
                                            'account_id':self.other_deduct_acc.id,
                                            'name':name_other_deduction_fees+self.loan_id,
                                            'debit':0.0,
                                            'credit':self.other_deduct,
                                            'partner_id':self.partner_id.id,
                        })
                else:
                    other_deduction_fee.update({
                ...
                list_mv_line.append((0, 0, other_deduction_fee))
            ...
            if not amount_currency:
                ...
                if flag:
                    move_lines_cr['credit'] = disburse_amt - self.process_fee - self.insure_fee - self.tra_fee - self.tracking_fee -self.parking_fee-self.other_deduct
            else:
                move_lines_cr.update({
                                'account_id':self.journal_disburse_id.default_credit_account_id.id,
                                'name':name_bank_deposite+ self.loan_id,
                ...
                if flag:
                    ...
                    move_lines_cr['amount_currency'] = -amount_currency - (-self.process_fee) - (-self.insure_fee)- (-self.tra_fee)- (-self.tracking_fee)- (-self.parking_fee)- (-self.other_deduct)
    ...
    def _simple_interest_get(self, inter_rate):
        ...
        for num in self.installment_id:
            numbering += 1
        for i in range(numbering,self.total_installment+1):
            interest_month = round(((total * rate_interest)))
...
#         d_l = self.read(['apply_date'])[0]
        ...
        tr_fee = self.tra_fee
        car_fee = self.tracking_fee
        park_fee = self.parking_fee
        deduct_fee = self.other_deduct
        ins_fee = self.insure_fee
        ...
        try:
            ...
            move_vals3 = {
                         ...
                         'name': acc_move_line_name,
                         'date': curr_date,
                         'account_id': sal_obj.insure_fee_acc.id,
                         'debit': ins_fee ,
                         'credit' : 0.0,
                         'journal_id' : 5
                         } 
            move_vals4 = {
                         ...
                         'name': acc_move_line_name,
                         'date': curr_date,
                         'account_id': sal_obj.tra_fee_acc.id,
                         'debit': tr_fee ,
                         'credit' : 0.0,
                         'journal_id' : 5
                         } 
            move_vals5 = {
                         ...
                         'name': acc_move_line_name,
                         'date': curr_date,
                         'account_id': sal_obj.tracking_fee_acc.id,
                         'debit': car_fee ,
                         'credit' : 0.0,
                         'journal_id' : 5
                         }
            ...
            move_vals6 = {
                         ...
                         'name': acc_move_line_name,
                         'date': curr_date,
                         'account_id': sal_obj.parking_fee_acc.id,
                         'debit': park_fee ,
                         'credit' : 0.0,
                         'journal_id' : 5
                         }
            move_vals7 = {
                         ...
                         'name': acc_move_line_name,
                         'date': curr_date,
                         'account_id': sal_obj.other_deduction_fee.id,
                         'debit': deduct_fee ,
                         'credit' : 0.0,
                         'journal_id' : 5
                         }
            ...
            self.env['account.move'].create({'name': '/', 'partner_id': self.partner_id.id, 'journal_id': journal_id.id, 'date': curr_date, 'line_ids':[(0, 0, move_vals), (0, 0, move_vals1), (0, 0, move_vals2), (0, 0, move_vals3), (0, 0, move_vals4), (0, 0, move_vals5), (0, 0, move_vals6), (0, 0, move_vals7)]})
        except:
    ...
    def reject_loan(self):
        if not self.reject_resons:
            raise osv.except_osv(
                    'Could not Reject Loan!',
                     'You must fill Rejection Resons.')
...
#                 raise UserError("Please provide value for : \n\n- Customer Loan Account,\n- Interest Account,\n- Processing Fee Account, \n- Bank Account")
    ...
    def approve_finance(self):
        ...
        if self.loan_amount <= 0:
            raise UserError("Approved amount cannot be \"0.00\"")
...
#         self.apply_loan()
    ...
    def Submit_to_manager(self):
        if not self.req_amt:
            raise UserError("Dear Officer, please provide Requested Amount")

        if not self.proposed_amount:
            raise UserError("Dear Officer, please provide Amount you propose for your client as of assisment results")

        if not self.proof_id:
            raise UserError("Dear Officer, please provide Supporting Document")

        
        if not self.collateral_lines:
                raise UserError("Warning :Dear Officer , please Provide Collateral Informations")
        else:
...
class WritteOff(models.Model):
    _name = 'writte.off'
    _description = 'Writte Off'
    _rec_name = 'loan_id'

    loan_id = fields.Many2one('account.loan')
    loan_amount = fields.Float(string = 'Loan Amount' , related='loan_id.loan_amount')
    client = fields.Many2one(string = 'Customer Name' , related='loan_id.partner_id')

    loan_type = fields.Many2one(string = 'Loan Type' , related='loan_id.loan_type')
    writteoff_date = fields.Date('Writte Off Date')
    writteoff_amount = fields.Float('Writte Off Amount')
    loan_aim = fields.Text(string = 'Loan Purpose' , related='loan_id.name')
    reasons = fields.Text(string = 'Writte off Reasons' )
    ...
    jornal_name = fields.Many2one('account.journal' , sstring = 'Jornal Name')
    ...
    #     res = super(WritteOff, self).default_get(fields)
    #     res['credit_account'] = self.env.user.employee_id.id
    #     res['debit_account'] = 147
    ...
    def action_submit(self):
        if not self.writteoff_amount:
            raise UserError("Dear User, please provide  Amount to Writte Off")

        if not self.writteoff_date:
            raise UserError("Dear User, please provide Writte of Date")

        if not self.credit_account:
            raise UserError("Dear User, please provide Credit Account")

        if not self.debit_account:
            raise UserError("Dear User, please provide Debit Account")

        if not self.jornal_name:
            raise UserError("Dear User, please provide Jornal Name")
    ...
    def action_approved(self):
        ...
        if not self.writteoff_amount:
            raise UserError('You must Indicate Amount you want to writte Off')
            
        else:
            if not self.debit_account or not self.credit_account or not self.jornal_name:
                raise UserError("You must enter Debit account & Credit account and journal to approve ")
           
            timenow = time.strftime('%Y-%m-%d')
            for loanoff in self:
                amount = loanoff.writteoff_amount
                loan_name = loanoff.client.name
                reasons = loanoff.reasons
                loan_client_id = loanoff.client.id
                reference = loanoff.loan_id.id
                journal_id = loanoff.jornal_name.id
                debit_account_id = loanoff.debit_account.id
                credit_account_id = loanoff.credit_account.id
                debits_vals = {
                    'name': reasons,
                ...
                valss = {
                    'name': 'Writte off loan for ' + ' ' + loan_name,
                    'narration': loan_name,
                    'ref': reference,
                    'journal_id': journal_id,
                    'date': timenow,
                    'line_ids': [(0, 0, debits_vals), (0, 0, credits_vals)]
                }

                x_move = self.env['account.move'].create(valss)
                x_move.post()
                self.write({'state': 'approve'})
                loan_obj = self.env['account.loan'].search([(1,'=',1)])
                retz=0
                for xyzd in loan_obj:
                    if xyzd == loanoff.loan_id:
                        self.env['account.loan'].search([('id','=',xyzd.id)]).write({'state':'off'})
                        break
...
class LoanCollection(models.Model):
    ...
    _description = "Loan Collections from Writte off"
    ...
    account_id = fields.Many2one('account.account', string='Writte Off Account',default=lambda self:50, readonly=True)
    credit_account = fields.Many2one('account.account', string='Writte Off Expense Account')



    payment_date = fields.Date(string='Payment Date', default=fields.Date.context_today, required=True, copy=False)
    loan_id = fields.Many2one('account.loan', "Loan ID.")
    emp_account_id = fields.Many2one('account.account', string="Loan Account")
    employee_id = fields.Many2one('res.partner', string="Client Name", required=True)
    ...
    @api.model
    def default_get(self, fields):
        rec = super(LoanCollection, self).default_get(fields)
        if not self._context:
            return rec
        active_id = self._context.get('active_id')
        loan_id = self.env[self._context.get('active_model')].browse(active_id)
        if loan_id.jornal_name:
            rec['journal_id'] = loan_id.jornal_name.id
            rec['loan_id'] = loan_id.id
            rec['amount'] = loan_id.loan_amount
            #rec['emp_account_id'] = loan_id.emp_account_id.id
            
            rec['employee_id'] = loan_id.client.id
            rec['credit_account'] = loan_id.debit_account.id
    ...
    def _onchange_disburse_journal(self):
        if self.journal_id:
            active_id = self._context.get('active_id')
            acc_loan = self.env['hr.loan'].browse(active_id)
    ...
    def action_validate_loan_payment(self):
        ...
        if self.amount == 0.0:
            ...
            if not self.amount or not self.payment_date or not self.journal_id:
                raise UserError("You must Must make sure all fields are filled ")
           
            name = self.name
            amount = self.amount
            dates = self.payment_date
            journal_id = self.journal_id
            debit_account = self.account_id.id
            credit_account = self.credit_account.id
            
            loan_client_id = self.employee_id
            #reference = self.name
            timenow = time.strftime('%Y-%m-%d')
            ...
            valss = {
                'name': 'Employee Loan Repayment ' + ' ' + loan_client_id.name,
                'narration': loan_client_id.name,
                # 'ref': reference,
                'journal_id': journal_id.id,
                'date': timenow,
                'line_ids': [(0, 0, debits_vals), (0, 0, credits_vals)]
            ...
            loan_obj = self.env['hr.loan'].search([('name','=',name)])
            xyz = loan_obj.total_paid_amount + amount

            new_blnc = loan_obj.total_amount - xyz
            loan_obj.write({'total_paid_amount':xyz , 'balance_amount':new_blnc })
